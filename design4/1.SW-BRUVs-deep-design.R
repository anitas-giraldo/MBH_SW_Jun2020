...
writeRaster(bd, "~/MBHdesignSW/spatial_data/SW60m_reefs.tif", overwrite=TRUE)
b2 <- raster("~/MBHdesignSW/spatial_data/SW60m_reefs.tif")
b2
plot(b2)
b2 <- raster::aggregate(b2, factor=2)
####################################
...
Bathy.quant <- c(0,0.25,0.5,0.95,1)
...
writeRaster(catB, "~/MBHdesignSW/design1/Bathy_cuts_60mSW1.tif", overwrite=TRUE)
...
######### Get TPI #############
plot(b)
tpi <- terrain(b2, opt="TPI")
plot(tpi)
b3 <- raster::aggregate(b2, fact=4)
plot(b3, add=T)
plot(b3)
writeRaster(tpi, "~/MBHdesignSW/spatial_data/TPI60mreefs12x12.tif")
tpi <- raster("~/MBHdesignSW/spatial_data/TPI60mreefs12x12.tif")
plot(tpi)
...
breaks <- seq ( from= min ( bathydf$depth, na.rm= TRUE ),
                to= max ( bathydf$depth, na.rm= TRUE ), length= nbins +1 ) # -2.1618613 -0.7142973  0.7332666  2.1808306
# chech the values above with raster
minValue(b2$SW60m_reefs) # -2.161861
maxValue(b2$SW60m_reefs) # 2.180831
...
tmpHist <- hist ( bathydf$depth, breaks= Bathy.cuts, plot= T, freq = F )
...
pdf( "~/MBHdesignSW/design4/Clusters60m_designSW.pdf", height=7, width=8)
...
pdf( "~/MBHdesignSW/design3/Clusters60m_designSW.pdf", height=7, width=8)
plot(rasterIP)
plot(b2)
points(fullSample3, pch=20, col='black')
points(clustersp, pch=20, col='blue')
...
writeOGR( fullSample3, dsn="~/MBHdesignSW/design4", layer=paste( "Bruvs60m", Sys.Date(), sep="_"), driver="ESRI Shapefile", overwrite_layer=TRUE)
writeOGR( clustersp, dsn="~/MBHdesignSW/design4", layer=paste( "clusters60m", Sys.Date(), sep="_"), driver="ESRI Shapefile", overwrite_layer=TRUE)
